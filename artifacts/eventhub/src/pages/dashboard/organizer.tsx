...
const defaultDashboardData = {
  ...
  totalCheckins: 310,
  recentActivity: [
    { id: 1, message: "New attendee registration for TechRush Hackathon", timestamp: new Date().toISOString() },
    { id: 2, message: "Volunteer shift assigned to Nehal Ahuja", timestamp: new Date(Date.now() - 3600000).toISOString() },
    { id: 3, message: "Event approval received from Admin (Tanishka Ghewari)", timestamp: new Date(Date.now() - 7200000).toISOString() },
  ],
  upcomingEvents: [
    {
      id: 1,
    ...
    {
      id: 2,
      title: "AI & Cloud Architecture Summit",
      category: "Conference",
...
export default function OrganizerDashboard() {
  const { data: serverData } = useGetOrganizerDashboard({
    query: {
      retry: false,
    },
  });

  const data = serverData || defaultDashboardData;
      ...
      <div className="space-y-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-6 border-b border-border/60 gap-4">
          ...
          <div className="flex items-center gap-3">
            <Link href="/dashboard/organizer/events/new">
              <Button className="font-bold text-xs shadow-xs h-10 px-5 rounded-xl cursor-pointer">
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: "Total Events", value: data?.totalEvents ?? 5, trend: "+12% this month", icon: Calendar, color: "text-primary bg-primary/10" },
              { label: "Published", value: data?.publishedEvents ?? 3, trend: "Active on Portal", icon: TrendingUp, color: "text-emerald-600 bg-emerald-500/10" },
              { label: "Registrations", value: data?.totalRegistrations ?? 480, trend: "+24% growth rate", icon: Users, color: "text-purple-600 bg-purple-500/10" },
              { label: "Verified Check-ins", value: data?.totalCheckins ?? 310, trend: "98.4% QR Accuracy", icon: CheckSquare, color: "text-amber-600 bg-amber-500/10" },
          ...
          <Card className="rounded-2xl border-border/60 shadow-xs p-6 bg-card">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Link href="/dashboard/organizer/events">
                <Button variant="ghost" size="sm" className="text-primary font-bold text-xs cursor-pointer">
            ...
            {(!data?.upcomingEvents || data.upcomingEvents.length === 0) ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {data.upcomingEvents.map(event => {
                  const fill = event.registeredCount ? Math.round((event.registeredCount / (event.capacity || 100)) * 100) : 0;
                  return (
                    <Card key={event.id} className="rounded-2xl border-border/60 hover:border-primary/40 transition-all shadow-xs p-5 space-y-4 bg-card">
                      <div className="flex justify-between items-start">
                        <Badge variant="outline" className="text-[10px] font-bold border-primary/30 text-primary">{event.category || "Event"}</Badge>
                        <span className="text-xs text-muted-foreground font-medium">{event.startTime ? format(new Date(event.startTime), "MMM d, yyyy") : "Upcoming"}</span>
                      </div>
                      ...
                      <div className="space-y-1.5 text-xs">
                        <div className="flex justify-between text-muted-foreground font-semibold">
                          ...
                          <span className="text-primary font-bold">{event.registeredCount || 0} / {event.capacity || 100} ({fill}%)</span>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 pt-1">
                        <Link href={`/dashboard/organizer/events/${event.id}/attendance`}>
                          <Button variant="outline" size="sm" className="w-full text-xs font-bold rounded-xl h-9 cursor-pointer">
          ...
          {data?.recentActivity && data.recentActivity.length > 0 && (
            <div className="space-y-4">
              ...
              <Card className="rounded-2xl border-border/60 shadow-xs overflow-hidden bg-card">
