/**
 * Frontend & Universal AI Event Mascot Synthesis Engine
 */
...
export const MASCOT_PROFILES: Record<string, MascotProfile> = {
  technology: {
    name: "Byte the Cyber Owl",
    archetype: "Cybernetic Owl",
...
export function synthesizeMascot(title?: string, category?: string, keywords?: string): {
  mascotName: string;
  personality: string;
  category: string;
  prompt: string;
  ...
  const catInput = (category || "").toLowerCase();
  const combined = `${title || ""} ${category || ""} ${keywords || ""}`.toLowerCase();

  let matchedKey = "technology";

  if (catInput === "cultural" || catInput === "arts" || catInput === "music" || combined.includes("music") || combined.includes("art") || combined.includes("dance") || combined.includes("cultural")) {
    matchedKey = "cultural";
  } else if (catInput === "sports" || combined.includes("sports") || combined.includes("cricket") || combined.includes("athletics") || combined.includes("fitness")) {
    matchedKey = "sports";
  } else if (catInput === "business" || catInput === "entrepreneurship" || combined.includes("startup") || combined.includes("pitch") || combined.includes("leadership")) {
    matchedKey = "business";
  } else if (catInput === "science" || catInput === "academic" || combined.includes("science") || combined.includes("research") || combined.includes("lab") || combined.includes("workshop")) {
    matchedKey = "science";
  } else {
    matchedKey = "technology";
  }

  const profile = MASCOT_PROFILES[matchedKey] || MASCOT_PROFILES.technology;

  // Convert SVG string to base64 Data URL
  let mascotUrl: string;
  if (typeof window !== "undefined" && typeof btoa === "function") {
    mascotUrl = `data:image/svg+xml;base64,${btoa(profile.svgIcon)}`;
  } else {
    mascotUrl = `data:image/svg+xml;base64,${Buffer.from(profile.svgIcon).toString("base64")}`;
  }

  const dynamicPrompt = title ? `${profile.prompt} Specifically customized for '${title}'.` : profile.prompt;
